        ...
        (async () => {
            ...
            request(options, async function (error, response) {
                ...
                if (url.parse(req.url, true).query.bgimg) {
                    ...
                    if (thisistoday) {
                        let headercap = '<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link href="https://fonts.googleapis.com/css2?family=Mitr&display=swap" rel="stylesheet"><style>body{font-family: \'Mitr\', font-noto-thai;background-image: url(\'http://localhost:' + process.env.PORT + '/'+bgurl+'\');color: white;}</style></head>'

                        await new Pageres({ format: 'jpg', delay: 1, filename: datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8), launchOptions: { executablePath: '/usr/bin/chromium-browser', args: ['--no-sandbox', '--disable-setuid-sandbox', '--no-first-run', '--disable-extensions'] } })
